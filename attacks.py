# -*- coding: utf-8 -*-
"""
Created on Sat Dec 16 18:25:00 2023
...
def add_salt_and_pepper_noise(image, salt_percentage=0.01, pepper_percentage=0.01):
    row, col, ch = image.shape
    noisy = np.copy(image)

    # Salt noise
    num_salt = int(salt_percentage * image.size)
    salt_coords = [np.random.randint(0, i - 1, num_salt) for i in image.shape]
    noisy[salt_coords] = 255

    # Pepper noise
    num_pepper = int(pepper_percentage * image.size)
...
plt.subplot(334), plt.imshow(noisy_image_salt_pepper[..., 0], cmap='gray'), plt.title('Salt & Pepper Noise')
plt.xticks([]), plt.yticks([])

plt.subplot(335), plt.imshow(filtered_image_salt_pepper[..., 0], cmap='gray'), plt.title('Salt & Pepper Filtered')
plt.xticks([]), plt.yticks([])
